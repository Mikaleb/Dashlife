...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const localStorageUser = computed<any>(
      () => ctx.root.$store.getters['getUser']
    )
    ...
    user.map((data) => {
      ...
      if (Object.keys(localStorageUser.value).length) {
        obj[data.input.id] = localStorageUser.value[data.input.id]
      }
